...
info() {
    ...
    tcp | kcp | quic)
        ...
        is_vmess_url=$(jq -c '{v:2,ps:'\"${net}\"',add:'\"$is_addr\"',port:'\"$port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',type:'\"$header_type\"',path:'\"$kcp_seed\"'}' <<<{})
    ...
    ws | h2 | grpc)
        ...
        [[ $is_protocol == 'vmess' ]] && {
            ...
            is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&${is_url_path}=$(sed 's#/#%2F#g' <<<$path)#$net"
        }
